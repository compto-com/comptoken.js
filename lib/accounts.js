...
const { getOptionOr, numAsU16ToLEBytes, toOption, normalizeTime } = require("./utils.js");
...
/**
 * @template EnumType
 * @typedef {EnumType[keyof EnumType]} EnumValues
 */
...
class TLV {
    ...
    static fromBytes(bytes) {
        ...
        return new TLV(buffer.getUint16(0, true), buffer.getUint16(2, true), Uint8Array.from(bytes).subarray(4, 4 + buffer.getUint16(2, true)));
    }
}
 ...
 *      LAYOUT: Layout<DataT>,
 *      fromBytes: (bytes: Uint8Array | Buffer) => DataType<DataT>,
 * }} DataTypeStatic
 */
...
function fromBytesDataType(dataType, bytes) {
    return new dataType(dataType.LAYOUT.decode(Uint8Array.from(bytes)));
}
 ...
 *          SIZE: number,
 *          ACCOUNT_TYPE: number,
 *          decodeExtensions: (buffer: Uint8Array | Buffer) => TLV[]
 ...
 *      fromAccountInfoBytes: (address: PublicKey, accountInfo: AccountInfo<Uint8Array | Buffer>) => Account<DataT>,
 * }} AccountStatic
 */
...
const UserDataLayout = struct([
    ...
    u64("inactiveInterest"),
    u64("inactiveUbiInterest"),
    u64("inactiveUbi"),
    u64("length"),
 ...
 * @property {bigint}       verificationDate
 * @property {Uint8Array}   nullifierHash
 * @property {bigint}       inactiveInterest
 * @property {bigint}       inactiveUbiInterest
 * @property {bigint}       inactiveUbi
...
class UserData {
    ...
    static MIN_SIZE = UserDataLayout.fields.filter((v) => v.property != "proofs").reduce((p, c) => p + c.span, 0) + 32; // min 1 proof

    /**
     * @param {object}       params 
     * @param {bigint}       params.lastInterestPayoutDate
     * @param {bigint}       params.verificationDate
     * @param {Uint8Array}   params.nullifierHash
     * @param {bigint}       params.inactiveInterest
     * @param {bigint}       params.inactiveUbiInterest
     * @param {bigint}       params.inactiveUbi
    ...
    constructor({
        ...
        this.inactiveInterest = inactiveInterest;
        this.inactiveUbiInterest = inactiveUbiInterest;
        this.inactiveUbi = inactiveUbi;
        this.length = length;
    ...
    static MAX_REVERIFICATION_WAIT = 60 * 60 * 24 * 31; // 1 month (31 days) in seconds

    /** @readonly */
    static VerificationStates = Object.freeze({
     ...
     * @returns {EnumValues<typeof UserData.VerificationStates>}
     */
    static getVerificationState(userData) {
        const todaySec = Math.floor(normalizeTime(new Date()).getTime() / 1000);
        if (userData.verificationDate == 0n) {
            return UserData.VerificationStates.UNVERIFIED;
        } else if (userData.verificationDate >= todaySec - UserData.MAX_REVERIFICATION_WAIT) {
            return UserData.VerificationStates.VERIFIED;
        } else {
            return UserData.VerificationStates.STALE;
        }
    }
}
 ...
 * @property {bigint}         inactiveVerifiedHumans
 * @property {bigint}         totalInactiveComptokens
 * @property {bigint}         burnedComptokens
...
const DailyDistributionDataLayout = struct([
    ...
    u64("verifiedHumans"),
    u64("inactiveVerifiedHumans"),
    u64("totalInactiveComptokens"),
    u64("burnedComptokens"),
