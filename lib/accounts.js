...
const { DAILY_DISTRIBUTION_HISTORY_SIZE, SEC_PER_DAY, compto_public_keys, ComptoPublicKeys } = require("./constants.js");
...
const publickKeys = compto_public_keys;
...
class UserDataAccount extends Account {
     ...
     * @returns {PublicKey}
     */
    static addressFromComptokenAccount(comptokenAccountAddress, compto_public_keys = publickKeys) {
...
async function getUserData(
    connection,
    user_comptoken_token_account_address,
    compto_public_keys = publickKeys,
    commitment = null,
) {
...
async function getValidBlockhashes(
    ...
    try {
        return getValidBlochashesFromTransactionResponse(result);
    } catch (error) {
 ...
 */
function getValidBlochashesFromTransactionResponse(transactionResponse) {
    let data = transactionResponse?.meta?.returnData[0];
