...
import { ACCOUNT_SIZE, AccountLayout, TokenAccountNotFoundError, TokenInvalidAccountOwnerError } from "@solana/spl-token";
import { Connection, PublicKey } from "@solana/web3.js";

import { GlobalDataAccount, TokenAccount, UserDataAccount } from "./accounts.js";
import { compto_program_id_pubkey, DAILY_DISTRIBUTION_HISTORY_SIZE, global_data_account_pubkey } from "./constants.js";
import { getOptionOr, normalizeTime, numAsU16ToLEBytes, ringBuffer, toOption } from "./utils.js";
 ...
 */
async function getComptokenAccount(connection, user_comptoken_token_account_address) {
    const accountInfo = await connection.getAccountInfo(user_comptoken_token_account_address);
    if (accountInfo === null) {
        throw new TokenAccountNotFoundError("User comptoken token account does not exist");
    }
    return TokenAccount.fromAccountInfoBytes(user_comptoken_token_account_address, accountInfo);
}
 ...
 */
export async function getComptokenBalance(connection, user_comptoken_token_account_address) {
    const userComptokenAccount = await getComptokenAccount(connection, user_comptoken_token_account_address);
 ...
 */
export async function getNominalOwner(connection, user_comptoken_token_account_address) {
    const userComptokenAccount = await getComptokenAccount(connection, user_comptoken_token_account_address);
 ...
 * @throws {TokenError}
 */
async function getUserData(connection, user_comptoken_token_account_address) {
    ...
    const user_data_account = await getAccount(connection, user_data_account_address, UserDataAccount);
    if (user_data_account.owner !== compto_program_id_pubkey) {
 ...
 */
export async function getDistributionOwed(connection, user_comptoken_token_account_address) {
    ...
    let ubiOwed = 0;
    let i = 0;
    for (let historicDistribution of await getHistoricDistributions(connection)) {
        if (i++ < (GlobalData.DAILY_DISTRIBUTION_HISTORY_SIZE - daysSinceLastPayout)) {
            continue;
        }
        interestOwed *= historicDistribution.interestRate;
        ubiOwed += historicDistribution.ubiAmount;
    }

    if (!isVerifiedHuman(connection, user_comptoken_token_account_address)) {
        ubiOwed = 0;
    }
...
export async function getDaysSinceLastPayout(connection, user_comptoken_token_account_address) {
    ...
    const daysSinceLastPayout = Math.min(
        ...
        normalizeTime(new Date()).valueOf() - user_data.last_interest_payout_date
    );
 ...
 * @throws {TokenError}
 */
export async function isVerifiedHuman(connection, user_comptoken_token_account_address) {
    const user_data = await getUserData(connection, user_comptoken_token_account_address);
...
/**
 * @param {Connection} connection 
 * @returns {Promise<import("../index.js").GlobalData>}
 */
async function getGlobalData(connection) {
    const global_data_account = await getAccount(connection, global_data_account_pubkey, GlobalDataAccount);
    return global_data_account.data;
}

/**
 * @param {Connection} connection 
 * @returns {Promise<Promise<import("../index.js").ValidBlockhashes>>}
 */
export async function getValidBlockhashes(connection) {
 ...
 */
export async function getHistoricDistributions(connection) {
    const globalData = await getGlobalData(connection);
    return function* () {
        ...
        for (const historicValue of ringBuffer(
            ...
            oldestHistoricValue,
            oldestHistoricValue + DAILY_DISTRIBUTION_HISTORY_SIZE)
        ) {
            yield historicValue;
        }
    }();
