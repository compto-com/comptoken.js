...
class ComptoPublicKeys {
    ...
    /** @type {PublicKey} */ world_id_bridge_program_id_pubkey;
     ...
     * @param {PublicKey?} params.world_id_bridge_program_id_pubkey
    ...
    constructor({
        ...
        world_id_bridge_program_id_pubkey,
        test_account,
    }) {
        ...
        if (!world_id_bridge_program_id_pubkey) {
            throw new Error('world_id_bridge_program_id_pubkey is required');
        }
        ...
        this.world_id_bridge_program_id_pubkey = world_id_bridge_program_id_pubkey ?? PublicKey.findProgramAddressSync([Buffer.from("world-id-bridge")], compto_program_id_pubkey)[0];
    ...
    /**
     * @template T
     * @typedef {T extends Keypair ? 'keypair' : T extends PublicKey ? 'publicKey' : never} _KeyType
     */

    /**
     * @typedef {{
     *      [Key in keyof ComptoPublicKeys]: _KeyMapping<_KeyType<ComptoPublicKeys[Key]>>
     * }} KeyMappings
     */

    static defaultKeymappings = /** @type {const} */ ({
        compto_program_id_pubkey: {
        ...
        world_id_bridge_program_id_pubkey: {
            filename: 'world_id_bridge_program_id.json',
            keyProperty: 'programId',
            type: 'publicKey',
        },
        test_account: {
    ...
    static loadFromCache(cacheDir, keyMappings = ComptoPublicKeys.defaultKeymappings) {
        /** @type {{[Key in keyof ComptoPublicKeys]?: PublicKey | Keypair}} */ const keys = {};
        ...
        for (const [key, mapping] of Object.entries(keyMappings)) {
            const keyName = /** @type {keyof ComptoPublicKeys} */ (key);
            ...
            switch (mapping.type) {
                ...
                case 'publicKey':
                    ...
                    break;
                default:
                    /* @ts-expect-error */// mapping.type should be never
                    throw new Error(`Unknown key type '${mapping.type}' for key '${keyName}'`);
            }
        }

        return new ComptoPublicKeys(/** @type {{[Key in keyof ComptoPublicKeys]: ComptoPublicKeys[Key]}} */(keys));
    }

    /**
     * @typedef {{[key: string]: any}} _Object
     */

    /**
     * Get a nested property from an object using a dot-separated string path
     * @param {object} obj - The object to get the property from
     * @param {string} propertyPath - The dot-separated string path to the property
     * @returns {any} - The value of the property, or undefined if not found
     */
    static #getNestedProperty(obj, propertyPath) {
        return propertyPath.split('.').reduce((o, p) => (o ? /** @type {_Object} */ (o)[p] : undefined), obj);
    }
}
...
let compto_public_keys = new ComptoPublicKeys({
    ...
    world_id_bridge_program_id_pubkey: new PublicKey(bs58.decode("")),
    test_account: null,
});
...
let devnet_compto_public_keys = new ComptoPublicKeys({
    ...
    world_id_bridge_program_id_pubkey: new PublicKey(bs58.decode("9TMVfMJs6qyu8jnc7TJfAWhn81Ju2uSRj4uYqLHyKXnh")),
    test_account: null,
});
