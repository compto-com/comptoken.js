...
const { DAILY_DISTRIBUTION_HISTORY_SIZE, SEC_PER_DAY, compto_public_keys: public_keys } = require("./constants.js");
...
/**
 * @import { Commitment, Connection, Keypair, PublicKey, VersionedTransactionResponse } from "@solana/web3.js";
 * 
 * @import { Account, AccountStatic, IUserData, IGlobalData } from "./accounts.js";
 * 
 ...
 * @param {AccountStatic<DataT>}    type
 * @param {Commitment}             [commitment]
 * @returns {Promise<Account<DataT>>}
 */
async function getAccount(connection, address, type, commitment = "confirmed") {
...
async function getNominalOwner(
    ...
    const userComptokenAccount = await getComptokenAccount(connection, user_comptoken_token_account_address, commitment);
    return userComptokenAccount.data.owner;
}
...
async function getUserData(
    ...
    const user_data_account = await getAccount(connection, user_data_account_address, /** @type {AccountStatic<IUserData>} */(UserDataAccount), commitment);
...
async function getDistributionOwed(
    ...
    let balance = Number(userComptokenAccount.data.amount);
    let ubiOwed = 0;
    let i = 0;
    ...
    const interestOwed = balance - Number(userComptokenAccount.data.amount) - ubiOwed;
...
async function isVerifiedHuman(
    ...
    const user_data = await getUserData(connection, user_comptoken_token_account_address, compto_public_keys, commitment);
    return user_data.verificationDate > 0;
}
