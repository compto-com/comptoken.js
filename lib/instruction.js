...
class ComptokenProof {
    ...
    export class ComptokenProof {
        ...
        hash: Buffer;
        header: Buffer;
        static MIN_NUM_ZEROED_BITS: number = 20; // Example value
        ...
        constructHeader() {
            ...
            const coinbaseHash = ComptokenProof.doubleSHA256(this.extraData);
            const pubkeyHash = ComptokenProof.doubleSHA256(this.pubkey.toBuffer());
            const merkleRoot = ComptokenProof.doubleSHA256(Buffer.concat([coinbaseHash, pubkeyHash]));
            ...
            const header = Buffer.concat([
                versionLE,          // Version (4 bytes)
                prevHashLE,         // Previous Block Hash (32 bytes)
                merkleRootLE,       // Merkle Root Hash (32 bytes)
                timestampLE,        // Timestamp (4 bytes)
                nonceLE             // Nonce (4 bytes)
        ...
        // Generate the hash of the header (double SHA-256)
        generateHash(): Buffer {
            console.log("Generating hash");
            return ComptokenProof.doubleSHA256(this.header);
        }
        ...
        static leadingZeroes(hash: Buffer): number {
            ...
            for (const byte of hash) {
                if (byte === 0) {
                    ...
                    // Count the number of leading zero bits in the byte
    ...
    generateHash() {
        // Step 1: Create the first SHA-256 hash
        let hasher = createHash("sha256");
        hasher.update(this.pubkey.toBuffer());
        hasher.update(this.recentBlockHash);
        hasher.update(this.nonce);
        let firstHash = hasher.digest(); // First hash result
    
        // Step 2: Create the second SHA-256 hash based on the first hash
        let secondHasher = createHash("sha256");
        secondHasher.update(firstHash); // Hash the result of the first hash
    
        return secondHasher.digest(); // Return the final hash (double SHA-256)
    }
