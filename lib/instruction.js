 ...
 * @param {import("./constants.js").ComptoPublicKeys} compto_public_keys
 * @returns
 */
async function createVerifyHumanInstruction(
    ...
    const WORLD_ID_PROGRAM = new PublicKey(base58.decode("9QwAWx3TKg4CaTjHNhBefQeNSzEKDe2JDxL46F76tVDv"));
    const user_data_account_address = UserDataAccount.addressFromComptokenAccount(user_comptoken_wallet_address, compto_public_keys);
    const world_id_nullifier = PublicKey.findProgramAddressSync([Buffer.from("Nullifier"), nullifier_hash], compto_program_id_pubkey)[0];
    const world_id_root = PublicKey.findProgramAddressSync([Buffer.from("Root"), root_hash, [1]], WORLD_ID_PROGRAM)[0];
    const world_id_latest_root = PublicKey.findProgramAddressSync([Buffer.from("LatestRoot"), [0]], WORLD_ID_PROGRAM)[0];
    ...
    return new TransactionInstruction({
        ...
        data: Buffer.concat([
            Buffer.from([Instruction.VERIFY_HUMAN]),
            bigintAsU64ToBytes(
                BigInt(await connection.getMinimumBalanceForRentExemption(0)) // no space needed
            ),
            root_hash,
            nullifier_hash,
            proof,
