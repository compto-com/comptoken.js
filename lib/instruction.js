...
const { bigintAsU64ToBytes, isLittleEndian } = require("./utils.js");
...
async function createProofSubmissionInstruction(
    ...
    const user_data_account_address = UserDataAccount.addressFromComptokenAccount(user_comptoken_token_account_address);
...
async function createGetOwedComptokensInstruction(
    ...
    return new TransactionInstruction({
        ...
        keys: [
            ...
            {
                pubkey: UserDataAccount.addressFromComptokenAccount(interest_bank_account_pubkey),
                isSigner: false,
                isWritable: false,
            },
            // needed by the transfer hook program (doesn't really exist)
            {
                pubkey: UserDataAccount.addressFromComptokenAccount(verified_human_ubi_bank_account_pubkey),
                isSigner: false,
                isWritable: false,
            },
...
async function createGrowUserDataAccountInstruction(
    ...
    const user_data_account_address = UserDataAccount.addressFromComptokenAccount(user_comptoken_wallet_address);
    return new TransactionInstruction({
        programId: compto_program_id_pubkey,
        keys: [
            // the payer of the rent for the account
...
async function createVerifyHumanInstruction(
    ...
    return new TransactionInstruction({
        ...
        keys: [
            ...
            // needed by the transfer hook program (doesn't really exist)
            {
                pubkey: UserDataAccount.addressFromComptokenAccount(future_ubi_bank_account_pubkey),
                isSigner: false,
                isWritable: false,
            },
            // the owner of the Comptoken Token Account
