...
const WORLD_ID_VERIFICATION_TYPE = 1; // 1 = orb
...
/**
 * @param {Connection} connection
 * @param {PublicKey} user_data_account_address
 */
async function collectRequiresUserSignature(connection, user_data_account_address) {
    const accountInfo = await connection.getAccountInfo(user_data_account_address);
    if (accountInfo === null) {
        throw new Error("User data account does not exist");
    }

    const userDataAccount = UserDataAccount.fromAccountInfoBytes(user_data_account_address, accountInfo);
    return UserData.getVerificationState(userDataAccount.data) === UserData.VerificationStates.STALE;
}
...
async function createCollectInstruction(
    ...
    const user_data_account_address = UserDataAccount.addressFromComptokenAccount(user_comptoken_token_account_address, compto_public_keys);
    const requires_user_signature = await collectRequiresUserSignature(connection, user_data_account_address);
    return new TransactionInstruction({
        programId: compto_public_keys.compto_program_id_pubkey,
        keys: [
            ...
            { pubkey: user_wallet_address, isSigner: requires_user_signature, isWritable: false },
...
async function createVerifyHumanInstruction(
    ...
    const user_data_account_address = UserDataAccount.addressFromComptokenAccount(user_comptoken_token_account_address, compto_public_keys);
    const world_id_nullifier = PublicKey.findProgramAddressSync(
        [Buffer.from("Nullifier"), nullifier_hash],
        compto_public_keys.compto_program_id_pubkey
    )[0];
    const world_id_root = PublicKey.findProgramAddressSync(
        [Buffer.from("Root"), root_hash, Buffer.from([WORLD_ID_VERIFICATION_TYPE])],
        compto_public_keys.world_id_bridge_program_id_pubkey,
    )[0];
    const world_id_latest_root = PublicKey.findProgramAddressSync(
        [Buffer.from("LatestRoot"), Buffer.from([WORLD_ID_VERIFICATION_TYPE])],
        compto_public_keys.world_id_bridge_program_id_pubkey,
    )[0];
    const world_id_config = PublicKey.findProgramAddressSync([Buffer.from("Config")], compto_public_keys.world_id_bridge_program_id_pubkey)[0];
    return new TransactionInstruction({
        programId: compto_public_keys.compto_program_id_pubkey,
        keys: [
            ...
            { pubkey: compto_public_keys.world_id_bridge_program_id_pubkey, isSigner: false, isWritable: false },
            // TODO
 ...
 * @param {ComptoPublicKeys} compto_public_keys
 */
async function createFlagInactiveAccountInstruction(
    user_comptoken_token_account_address,
    compto_public_keys = public_keys,
) {
    const user_data_account_address = UserDataAccount.addressFromComptokenAccount(user_comptoken_token_account_address, compto_public_keys);
    return new TransactionInstruction({
        ...
        data: Buffer.from([Instruction.FLAG_INACTIVE_ACCOUNT]),
    });
}
...
async function createUnverifyHuman2Instruction(
    ...
    const world_id_config = PublicKey.findProgramAddressSync(
        [Buffer.from("Config")],
        compto_public_keys.world_id_bridge_program_id_pubkey
    )[0];
    return new TransactionInstruction({
        ...
        data: Buffer.concat([
            Uint8Array.from([Instruction.UNVERIFY_HUMAN_2]),
            Uint8Array.from(root_hash),
            Uint8Array.from(nullifier_hash),
            Uint8Array.from(proof),
        ]),
