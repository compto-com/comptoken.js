...
async function createVerifyHumanInstruction(
    ...
    const world_id_root = PublicKey.findProgramAddressSync([Buffer.from("Root"), root_hash, Buffer.from([WORLD_ID_VERIFICATION_TYPE])], WORLD_ID_PROGRAM)[0];
    const world_id_latest_root = PublicKey.findProgramAddressSync([Buffer.from("LatestRoot"), Buffer.from([WORLD_ID_VERIFICATION_TYPE])], WORLD_ID_PROGRAM)[0];
...
async function createReverifyHumanInstruction(
    ...
    return new TransactionInstruction({
        ...
        data: Buffer.concat([
            Buffer.from([Instruction.REVERIFY_HUMAN]),
            root_hash,
            nullifier_hash,
            proof,
        ]),
...
async function createUnverifyHumanInstruction(
    ...
    return new TransactionInstruction({
        ...
        data: Buffer.concat([
            Buffer.from([Instruction.UNVERIFY_HUMAN]),
        ]),
    });
}
